...
class ItemModel extends Model
{
    ...
    function __construct(){
        $this->tableName = 'item'; // 不包含前缀表名
        // $this->trueTableName = 'dp_itemb'; // 真实表名，如果不指定默认为模型名称
        parent::__construct();//继承父类初始化需要放在最后，否则上面的定义是无效的
    }

    public function search($keyword){
        return $this->sql("select * from item where `item_name` like '%$keyword%'");
    }
    public function getlist(){
        return $this->select('*','id<10');
    }
    public function add($data){
        return $this->insert($data,false);//第二个参数true返回insertid,false返回插入行数，默认true
    }
}